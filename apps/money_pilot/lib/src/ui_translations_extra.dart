const frenchUiPhrasesExtra = <String, String>{
  'Add a category guardrail to start planning.':
      'Ajoutez un budget de catégorie pour commencer à planifier.',
  'Add only the financial records you choose':
      'Ajoutez uniquement les données financières de votre choix',
  'Add recurring commitments to improve forecasts.':
      'Ajoutez les engagements récurrents pour améliorer les prévisions.',
  'Affordability needs an actual available balance. No balance is assumed.':
      'L’analyse nécessite un solde disponible réel. Aucun solde n’est supposé.',
  'Assumes this month’s income and expense pace continues, with current open bills recurring monthly.':
      'Suppose que le rythme des revenus et dépenses du mois continue et que les factures ouvertes se répètent chaque mois.',
  'Argon2id password hashing and recovery codes':
      'Hachage des mots de passe Argon2id et codes de récupération',
  'Cash flow needs review': 'La trésorerie doit être vérifiée',
  'Clarity for every\nmoney decision.':
      'De la clarté pour chaque\ndécision financière.',
  'Core account and finance tools work offline':
      'Les principaux outils financiers fonctionnent hors ligne',
  'Core tracking, budgets, forecasts, and the Local Coach work without internet.':
      'Le suivi, les budgets, les prévisions et le Coach local fonctionnent sans Internet.',
  'Deterministic insights calculated from your local snapshot.':
      'Analyses déterministes calculées à partir de vos données locales.',
  'Discuss with AI Coach': 'Discuter avec le Coach IA',
  'Due in 14 days': 'Échéance dans 14 jours',
  'Educational estimate—not financial, tax, investment, or legal advice.':
      'Estimation éducative, et non conseil financier, fiscal, juridique ou en investissement.',
  'Explain my safe to spend': 'Expliquer mon montant disponible',
  'Keep balances and safe-to-spend inclusion rules transparent.':
      'Gardez les soldes et les règles d’inclusion du montant disponible transparents.',
  'Keep it visibly provisional.': 'La garder visiblement provisoire.',
  'MoneyPilot uses integer cents and the more conservative of liquidity and remaining discretionary budgets.':
      'MoneyPilot calcule en centimes et retient la valeur la plus prudente entre la liquidité et les budgets discrétionnaires restants.',
  'No money will move and no transaction will be created.':
      'Aucun argent ne sera déplacé et aucune transaction ne sera créée.',
  'No sample balances, expenses, salary, bills, or goals are added for you.':
      'Aucun solde, dépense, salaire, facture ou objectif fictif n’est ajouté.',
  'Only include liquid balances you can actually use.':
      'Incluez uniquement les soldes disponibles que vous pouvez réellement utiliser.',
  'Open Budgets to tune a guardrail without changing history.':
      'Ouvrez Budgets pour ajuster une limite sans modifier l’historique.',
  'Pending authorizations already reduce available balances':
      'Les autorisations en attente réduisent déjà les soldes disponibles',
  'Search, categorize, and edit every local money movement.':
      'Recherchez, classez et modifiez chaque mouvement financier local.',
  'See commitments before they affect your available cash.':
      'Visualisez les engagements avant qu’ils n’affectent vos liquidités.',
  'See what is safe to spend, keep plans on course, and ask a coach that explains before it acts.':
      'Voyez ce que vous pouvez dépenser, gardez vos plans sur la bonne voie et consultez un Coach qui explique avant d’agir.',
  'Separate profiles with isolated local data':
      'Profils séparés avec données locales isolées',
  'Still count it as a protected commitment.':
      'Toujours la compter comme engagement protégé.',
  'This is the only way to reset a local password. Store it somewhere private; MoneyPilot cannot reveal it again.':
      'C’est le seul moyen de réinitialiser un mot de passe local. Conservez-le en lieu sûr ; MoneyPilot ne pourra pas le réafficher.',
  'This planning estimate is not financial advice. Review account inclusion and upcoming commitments when your situation changes.':
      'Cette estimation de planification n’est pas un conseil financier. Vérifiez les comptes inclus et les engagements à venir lorsque votre situation change.',
  'The purchase exceeds your current allowance. There is not enough recorded monthly surplus to estimate a catch-up date.':
      'L’achat dépasse votre montant disponible. L’excédent mensuel enregistré est insuffisant pour estimer une date de rattrapage.',
  'Turn longer-term plans into visible, manageable milestones.':
      'Transformez vos projets à long terme en objectifs visibles et réalisables.',
  'What bills are coming up?': 'Quelles factures arrivent bientôt ?',
  'What can you help me with?': 'Comment peux-tu m’aider ?',
  'Your estimate remains zero until you add your own accounts and obligations.':
      'Votre estimation reste à zéro jusqu’à l’ajout de vos comptes et obligations.',
  'Your safe-to-spend breakdown': 'Détail de votre montant disponible',
};

const arabicUiPhrasesExtra = <String, String>{
  'Add a category guardrail to start planning.':
      'أضف ميزانية لفئة كي تبدأ التخطيط.',
  'Add only the financial records you choose':
      'أضف فقط السجلات المالية التي تختارها',
  'Add recurring commitments to improve forecasts.':
      'أضف الالتزامات المتكررة لتحسين التوقعات.',
  'Affordability needs an actual available balance. No balance is assumed.':
      'يحتاج تقييم القدرة على الشراء إلى رصيد متاح حقيقي، ولا يتم افتراض أي رصيد.',
  'Assumes this month’s income and expense pace continues, with current open bills recurring monthly.':
      'يفترض استمرار معدل الدخل والمصروفات لهذا الشهر وتكرار الفواتير المفتوحة شهرياً.',
  'Argon2id password hashing and recovery codes':
      'تشفير كلمات المرور Argon2id ورموز الاسترداد',
  'Cash flow needs review': 'التدفق النقدي يحتاج إلى مراجعة',
  'Clarity for every\nmoney decision.': 'وضوح لكل\nقرار مالي.',
  'Core account and finance tools work offline':
      'أدوات الحساب والمالية الأساسية تعمل دون إنترنت',
  'Core tracking, budgets, forecasts, and the Local Coach work without internet.':
      'يعمل التتبع والميزانيات والتوقعات والمساعد المحلي دون إنترنت.',
  'Deterministic insights calculated from your local snapshot.':
      'تحليلات دقيقة محسوبة من بياناتك المحلية.',
  'Discuss with AI Coach': 'مناقشة مع المساعد المالي',
  'Due in 14 days': 'الاستحقاق خلال 14 يوماً',
  'Educational estimate—not financial, tax, investment, or legal advice.':
      'تقدير تعليمي وليس نصيحة مالية أو ضريبية أو استثمارية أو قانونية.',
  'Explain my safe to spend': 'اشرح المبلغ الآمن للصرف',
  'Keep balances and safe-to-spend inclusion rules transparent.':
      'حافظ على وضوح الأرصدة وقواعد إدراجها في المبلغ الآمن للصرف.',
  'Keep it visibly provisional.': 'أبقها ظاهرة كمعاملة مؤقتة.',
  'MoneyPilot uses integer cents and the more conservative of liquidity and remaining discretionary budgets.':
      'يستخدم MoneyPilot السنتات الصحيحة ويختار القيمة الأكثر تحفظاً بين السيولة وبقايا الميزانيات المرنة.',
  'No money will move and no transaction will be created.':
      'لن يتم نقل أي أموال ولن تُنشأ أي معاملة.',
  'No sample balances, expenses, salary, bills, or goals are added for you.':
      'لا تتم إضافة أرصدة أو مصروفات أو رواتب أو فواتير أو أهداف تجريبية نيابة عنك.',
  'Only include liquid balances you can actually use.':
      'أدرج فقط الأرصدة السائلة التي يمكنك استخدامها فعلياً.',
  'Open Budgets to tune a guardrail without changing history.':
      'افتح الميزانيات لضبط الحد من دون تغيير السجل.',
  'Pending authorizations already reduce available balances':
      'التفويضات المعلّقة تخفّض الأرصدة المتاحة بالفعل',
  'Search, categorize, and edit every local money movement.':
      'ابحث وصنّف وعدّل كل حركة مالية محلية.',
  'See commitments before they affect your available cash.':
      'شاهد الالتزامات قبل أن تؤثر في النقد المتاح.',
  'See what is safe to spend, keep plans on course, and ask a coach that explains before it acts.':
      'اعرف المبلغ الآمن للصرف، وحافظ على خططك، واسأل مساعداً يشرح قبل أن يتصرف.',
  'Separate profiles with isolated local data':
      'ملفات منفصلة وبيانات محلية معزولة',
  'Still count it as a protected commitment.': 'احتسابها دائماً كالتزام محمي.',
  'This is the only way to reset a local password. Store it somewhere private; MoneyPilot cannot reveal it again.':
      'هذه هي الطريقة الوحيدة لإعادة تعيين كلمة المرور المحلية. احفظ الرمز في مكان خاص لأن MoneyPilot لا يستطيع عرضه مجدداً.',
  'This planning estimate is not financial advice. Review account inclusion and upcoming commitments when your situation changes.':
      'هذا التقدير للتخطيط وليس نصيحة مالية. راجع الحسابات المشمولة والالتزامات القادمة عندما تتغير ظروفك.',
  'The purchase exceeds your current allowance. There is not enough recorded monthly surplus to estimate a catch-up date.':
      'يتجاوز الشراء المبلغ المتاح حالياً، ولا يوجد فائض شهري مسجل كافٍ لتقدير موعد تغطية الفرق.',
  'Turn longer-term plans into visible, manageable milestones.':
      'حوّل خططك طويلة الأجل إلى أهداف واضحة وقابلة للإدارة.',
  'What bills are coming up?': 'ما الفواتير القادمة؟',
  'What can you help me with?': 'بماذا يمكنك مساعدتي؟',
  'Your estimate remains zero until you add your own accounts and obligations.':
      'يبقى التقدير صفراً حتى تضيف حساباتك والتزاماتك.',
  'Your safe-to-spend breakdown': 'تفصيل المبلغ الآمن للصرف',
};
